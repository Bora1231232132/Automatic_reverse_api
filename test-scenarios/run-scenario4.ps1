# Content-Based Pairing Test - Scenario 4 (50,000 KHR)
...
Write-Host "  Content Pairing Test - Scenario 4" -ForegroundColor Cyan
Write-Host "  50,000 KHR | ext_ref $uniqueRef (unique this run)" -ForegroundColor Cyan
Write-Host "========================================" -ForegroundColor Cyan
Write-Host ""

# Step 1
Write-Host "[Step 1] Sending original: TOUR -> BKRT (50,000 KHR)..." -ForegroundColor Yellow
try {
    $xml1 = Get-Content -Path "test-scenarios/scenario4-small-amount/scenario4-step1-original.xml" -Raw -Encoding UTF8
    $xml1 = $xml1 -replace '3333222211', $uniqueRef
    $xml1 = $xml1 -replace 'CRTTOURKHPPXXX1738285000555', $msgId1
...
Write-Host "[Step 2] Sending reversal: BKRT -> TOUR (50,000 KHR, no REVERSING label)..." -ForegroundColor Yellow
try {
    $xml2 = Get-Content -Path "test-scenarios/scenario4-small-amount/scenario4-step2-reversal-no-label.xml" -Raw -Encoding UTF8
    $xml2 = $xml2 -replace '3333222211', $uniqueRef
    $xml2 = $xml2 -replace 'CRTBKRTKHPPXXX1738285100666', $msgId2
...
Write-Host "  Verify DB: SELECT * FROM transaction_logs WHERE amount = 50000;" -ForegroundColor Gray
Write-Host "========================================" -ForegroundColor Cyan
Write-Host ""
